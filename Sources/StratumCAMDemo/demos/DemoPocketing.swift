...
class DemoPocketing: Demo {
    ...
    func demoPocketRectangle() -> Demo.DemoResult {
        ...
        let strategy: SC.MachiningOperation = .pocket(direction: .climb, pocketType: .offsetPattern, entry: .plunge)

        return self.run(contour: rectangleContour(width: 40, height: 24), tool: tool, settings: settings, strategy: strategy)
    }
    ...
    func demoPocketRectangleConventional() -> Demo.DemoResult {
        ...
        let strategy: SC.MachiningOperation = .pocket(direction: .conventional, pocketType: .offsetPattern, entry: .plunge)
    ...
    func demoPocketRoundedRectangle() -> Demo.DemoResult {
        ...
        return self.run(contour: roundedRectangleContour(width: 40, height: 24, cornerRadius: 5), tool: tool, settings: settings, strategy: strategy)
    }
}
