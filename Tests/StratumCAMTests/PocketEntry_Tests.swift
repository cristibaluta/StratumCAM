...
struct PocketEntry_Tests {
    ...
    private func pocketStrategy(pocketType: SC.PocketType, entry: SC.EntryStrategy, direction: SC.CutDirection = .climb) -> SC.MachiningOperation {
        .pocket(direction: direction, pocketType: pocketType, entry: entry)
    }
    ...
    func testPocketRampEntryLandsOnRingStartAtDepth() {
        ...
        let toolpaths = engine.generateToolpaths(
            ...
            tool: tool,
            settings: settings,
            operation: pocketStrategy(pocketType: .offsetPattern, entry: .ramp(angleDegrees: 30))
        )
    ...
    func testPocketHelixEntryReturnsToRingStartAtDepth() {
        ...
        let toolpaths = engine.generateToolpaths(
            ...
            tool: tool,
            settings: settings,
            operation: pocketStrategy(pocketType: .offsetPattern, entry: .helix(radius: 1.0, rampAngleDegrees: 30))
        )
    ...
    func testRasterPocketHonorsRampEntry() {
        ...
        let toolpaths = engine.generateToolpaths(
            ...
            tool: tool,
            settings: settings,
            operation: pocketStrategy(pocketType: .raster, entry: .ramp(angleDegrees: 30))
        )
    ...
    func testRasterPocketHonorsHelixEntry() {
        ...
        let toolpaths = engine.generateToolpaths(
            ...
            tool: tool,
            settings: settings,
            operation: pocketStrategy(pocketType: .raster, entry: .helix(radius: 1.0, rampAngleDegrees: 30))
        )
    ...
    func testPocketPlungeEntryStillStraightDown() {
        ...
        let toolpaths = engine.generateToolpaths(
            ...
            tool: tool,
            settings: settings,
            operation: pocketStrategy(pocketType: .offsetPattern, entry: .plunge)
        )
